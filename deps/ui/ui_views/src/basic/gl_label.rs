...
impl ViewCallbacks for GLLabel {
    fn update(&mut self) {
...
pub trait AddGlLabel {
    fn add_label(&mut self, text: impl ToLabel) -> &mut Self;
}

impl<T: ?Sized + View> AddGlLabel for T {
    fn add_label(&mut self, text: impl ToLabel) -> &mut Self {
        let mut label = self.add_view::<GLLabel>();
