...
//     Color,
// };
// use ui::{refs::Address, UIManager, View, ViewData, ViewFrame, ViewSubviews};
...
//
// impl TEUIDrawer {
//     fn rounded_path_for_view<'a>(view: &dyn View, storage: &'a mut
// RoundStorage) -> &'a PathData {         if
// storage.get(&view.address()).is_some() {             let (path, prev_size) =
// storage.get_mut(&view.address()).unwrap();             if *prev_size ==
// view.frame().size {                 return path;
//             }
...
//     }
//
//     fn draw_image(&self, image: &GlImage, rect: &Rect, color: &Color,
// priority: u8, is_text: bool) {         draw_image(image, rect, color,
// priority, is_text);     }
//
//     fn draw_path(&self, path: &PathData, rect: &Rect, custom_mode:
// Option<DrawMode>, priority: u8) {         if rect.size.is_invalid() {
//             return;
//         }
//         self.set_viewport(rect);
//         BasicShaders::path()
...
//                 let frame = &image.size.fit_in_rect::<{ Axis::X
// }>(view.absolute_frame());                 self.draw_image(
//                     &image,
...
//         }
//
//         if let Some(drawing_view) =
// view.as_any().downcast_ref::<DrawingView>() {             for path in
// drawing_view.paths() {                 self.draw_path(path,
// view.absolute_frame(), None, view.priority());             }
//         }
//
//         //MARK - Debug frames
...
//             if view.dont_hide() ||
// view.absolute_frame().intersects(&self.root_frame) {                 
