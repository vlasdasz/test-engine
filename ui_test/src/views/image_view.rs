...
async fn check_pixel_color(_meter: Weak<ColorMeter>, _pos: Point, _color: Color) {
    from_main(move || {
...
async fn check_colors<const N: usize>(
    meter: SubView<ColorMeter>,
    data: [(impl Into<Point>, impl Into<Color>); N],
) {
    for val in data {
        check_pixel_color(meter.weak(), val.0.into(), val.1.into()).await
    }
}
